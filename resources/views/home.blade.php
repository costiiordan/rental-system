...
@section('content')
    <div>
        ...
        <form>
            ...
            <button type="submit">Caută biciclete disponibile</button>
        </form>
    </div>
            ...
            <div class="bike-item-details">
                ...
                <ul>
                    @foreach($bike->prices as $price)
                        <li>
                            {{$price->price}} RON / {{$price->duration}} {{$price->duration_unit}}
                        </li>
                    @endforeach
                </ul>
                @if($interval)
                    <div>
                        <p>
                            Pretul pentru perioada selectată: {{$prices[$bike->id]}} RON
                        </p>

                        <button data-from="{{$interval['from']->format('Y-m-d H:i')}}" data-to="{{$interval['to']->format('Y-m-d H:i')}}" data-bike-id="{{$bike->id}}" data-action="select-bike" class="rent-bike-button">
                            Închiriază
                        </button>
                    </div>
                @endif
